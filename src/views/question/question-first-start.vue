      ...
      <nav class="first-start-main-nav">
        ...
        <div class="queston-content single-question" v-if="single.singleSelection.length">
          ...
          <ul class="num-list clearfix">
            <li v-for="i in single.singleSelection.length" @click="changePage(i, '单选')" :class="single.singleIndex === i ? 'active' : ''" :key="i">
              {{ i }}<b class="peg" v-if="single.singleSelection[i - 1].tf_marked === 1"></b>
            </li>
          </ul>
        </div>
        <!-- 多选 -->
...
export default {
  ...
  created() {
    ...
    // 判断是否显示解析
    this.viewParsing = tiku_exercisesParams.tf_Practice === 1 ? true : false
    // this.tf_Practice_id = tiku_exercisesParams.tf_Practice
    // 获取模拟真题点击开始做题的id
    let test_paper_id = this.$route.query.test_paper_id
    // 章节练习的url
    let url = '/api/stu/ItemBank/issue'
    // 判断是不是模拟真题来的开始做题
    if (test_paper_id) {
      tiku_exercisesParams = {
        test_paper_id: test_paper_id,
        student_id: this.user_id // 用户id
      }
      url = '/api/stu/ItemBank/TestPaperInsertDoRecord'
    }
    axios({
      method: 'post',
      data: JSON.stringify(tiku_exercisesParams),
      url: url,
      headers: {
      ...
      // 获取总数
      this.notdone = this.total = res.data.length
      // 题分类
      res.data.forEach((el) => {
  ...
  methods: {
    ...
    successorerror(type = 'class') {
      if (type === 'class') {
        return this.multiple.multipleCheckbox.join(',') == this.multiple.multipleNow.real.join(',') ? true : false
      } else {
